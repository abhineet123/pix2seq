...
class Model(image_diffusion_model.Model):
    ...
    def noise_denoise(self, images, labels, time_step=None, training=True):
        ...
        """
        some sort of order inversion going on here since 
        video_panoptic_segmentation.infer_video passes (frames_i, cond_map_i) 
        to image_diffusion_model.call() which passes it unchanged here 
         """
        ...
        """sc = self conditioning"""
        if config.self_cond != 'none':
            sc_rate = config.get('self_cond_rate', 0.5)
            """config.self_cond_by_masking = False by default"""
            ...
            """
            this is a function defined inside get_cond_denoise which in turn calls 
            image_diffusion_model.denoise
            """
            cond_denoise = self.get_cond_denoise(
                labels[:num_sc_examples],
                cond_images[:num_sc_examples] if cond_images is not None else None)
            """
            hidden_shapes is a property in TapeDenoiser so cannot be None here 
            but can be so in image diffusion model so apparently this code is just copied from there"""
