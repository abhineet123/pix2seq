...
class ExplorerShell(cmd.Cmd):
    ...
    # https://stackoverflow.com/questions/8813291/better-handling-of-keyboardinterrupt-in-cmd-cmd-command-line
    # -interpreter
    ...
    def help_load(self):
        ...
        print(
            'Note: the operation is performed in-memory. To write changes back to the checkpoint, run `commit` after '
            '`load`.')
    ...
    def do_EOF(self, arg):
        if not arg:
            print('exit')
        if self._dirty:
            print('WARNING: there are pending mutations that have not been written to disk. Discard (y/N)? ', end='',
                  flush=True)
            ...
            if line == 'y' or line == 'yes':
                ...
                print(
                    'You can view the pending mutations with the `mutations` command or write them out to disk with '
                    '`commit`.')
                return
        print()
        return True
